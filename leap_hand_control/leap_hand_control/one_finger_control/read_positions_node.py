...
class ReadPositions(Node):
    def __init__(self):
        ...
        self.ax.set_ylabel("Posição do Motor (º)")
        self.ax.legend()
    ...
    def listener_callback(self, msg):
        ...
        #self.get_logger().info(f'Posições: {msg.data}')
    ...
    def update_plot(self):
        if len(self.timestamps) > 0:
            ...
            for motor, line in self.lines.items():
                angles = [(pos / 4095) * 360 for pos in self.motor_data[motor]]
                line.set_data(self.timestamps, angles)
