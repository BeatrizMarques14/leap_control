...
class ReadCurrents(Node):
    def __init__(self):
        super().__init__('read_currents')

        # Criar timestamp para o nome do ficheiro
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.csv_filename = f"/home/beatrix/ros2_ws/src/leap_control/leap_hand_control/leap_hand_control/data/currents/finger_currents_{timestamp}.csv"
    ...
    def listener_callback(self, msg):
        ...
        #self.get_logger().info(f'Correntes recebidas: {msg.data}')
    ...
    def update_plot(self):
        if len(self.timestamps) > 0:
            ...
            for motor, line in self.lines.items():
                line.set_data(self.timestamps, self.motor_data[motor])
